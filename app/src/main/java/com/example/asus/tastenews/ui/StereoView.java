...
public class StereoView extends ViewGroup {
    ...
    private static final int flingSpeed = 1000;
...
//                iStereoListener.stopFling();
    ...
    private void toNormalAction() {
        LogUtils.d("FLINGTEST","toNormalAction()");
        int startY;
        int delta;
        int duration;
        mState = State.Normal;
//        if(iStereoListener != null){
//            iStereoListener.stopFling(mCurScreen);
//        }
        addCount = 0;
        startY = getScrollY();
    ...
    private void toPreAction(float yVelocity) {
        LogUtils.d("FLINGTEST","toPreAction()");
        int startY;
        int delta;
        int duration;
        mState = State.ToPre;
    ...
    private void toNextAction(float yVelocity) {
        LogUtils.d("FLINGTEST","toNextAction()");
        int startY;
        int delta;
        int duration;
        mState = State.ToNext;
        ...
        if (mScroller.computeScrollOffset()) {
            if (mState == State.ToPre) {
                ...
                if (getScrollY() > (mHeight) && addCount > 0) {
                    isAdding = true;
                    LogUtils.d("FLINGTEST","computeScroll() and addCount = " + addCount);
                    addNext(true);
                    addCount--;
                    alreadyAdd++;
                }
...
//            iStereoListener.stopFling(mCurScreen);
     ...
     */
    private void addNext(boolean fromComputeScroll) {
        ...
        if (iStereoListener != null) {
            iStereoListener.toNext(mCurScreen,fromComputeScroll);
        }
    }
     ...
     */
    private void addPre(boolean fromComputeScroll) {
        ...
        addView(view, 0);
        if (iStereoListener != null) {
            iStereoListener.toPre(mCurScreen,fromComputeScroll);
        }
    }
    ...
    public interface IStereoListener {
        //上滑一页时回调
        void toPre(int curScreen,boolean fromComputeScroll);

        //下滑一页时回调
        void toNext(int curScreen,boolean fromComputeScroll);
    }
